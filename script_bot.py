...
"""
import json, os, random, time, urllib.request, urllib.parse, datetime
...
def make_script():
    ...
    narration = (
        f"[0-10s क्लिप 1 — हुक] {hook}\n"
        f"[10-20s क्लिप 2 — धमाल] {cat} {w} में दबे पाँव — {food} बुला रहा है। पंजा बाहर। आँखें बड़ी। मालिक सो रहा है। पकड़ा। मालिक जागा। पीछा। धमाल। बिल्ली vs इंसान।\n"
        f"[20-30s क्लिप 3 — एंड] बिल्ली जीती। पहला निवाला। लूप फ्रेम पर एंड।"
    )
...
def main():
    ...
    # Generate ONE script with 3 prompts (one per 10s clip = 30s video)
    script = make_script()
    ...
    # Send 3 prompts — one per 10s clip (Google Flow makes 10s videos)
    for i in range(3):
