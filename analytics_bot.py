...
def send_email(subject, body_html):
    """Send HTML email via Gmail SMTP only."""
    ...
    try:
        ...
        # Final fallback: create a GitHub Issue
        try:
            github_token = os.environ.get('GITHUB_TOKEN', '')
            if github_token:
                issue_body = f"# 📊 {subject}\n\n{body_html}\n\n---\n*This report was automatically generated by ToonPop Analytics Bot.*"
                ...
                issue_req = urllib.request.Request(
                    'https://api.github.com/repos/sameer-sys/video-autopost/issues',
                    data=issue_data,
                    headers={
                ...
                with urllib.request.urlopen(issue_req, timeout=15) as ir:
                    issue = json.load(ir)
                    print(f'✅ GitHub Issue created: #{issue.get("number")}')
                    return
