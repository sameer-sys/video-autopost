...
def send_email(subject, body_html):
    ...
    try:
        ...
        # Final fallback: create a GitHub Issue (GitHub emails repo owner automatically)
        try:
            import base64, json as json2
            github_token = os.environ.get('GITHUB_TOKEN', '')
            if not github_token:
                github_token = os.environ.get('GITHUB_TOKEN', '')  # try again
            repo = os.environ.get('GITHUB_REPOSITORY', 'sameer-sys/video-autopost')
            
            if github_token and 'github.com' not in str(type(github_token)):
                issue_body = f"""# 📊 {subject}

{body_html}

---
*This report was automatically generated by ToonPop Analytics Bot. Reply to this issue with actions you want me to take.*
"""
                issue_data = json2.dumps({
                    "title": f"📊 Analytics Report - {datetime.datetime.now().strftime('%Y-%m-%d')}",
                    "body": issue_body,
                ...
                with urllib.request.urlopen(issue_req, timeout=15) as ir:
                    issue = json2.load(ir)
                    print(f'✅ GitHub Issue created: #{issue.get("number")} - {issue.get("html_url")}')
                    print('   GitHub will email you about this issue.')
                    return
        except Exception as issue_err:
            print(f'❌ GitHub Issue fallback also failed: {issue_err}')

        # Last resort: save report locally
        print('📄 All email methods failed - saving report to analytics_email_backup.html')
